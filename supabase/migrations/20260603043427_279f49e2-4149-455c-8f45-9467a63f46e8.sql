-- Extend search_jobs to additively return has_screening
DROP FUNCTION IF EXISTS public.search_jobs(text,text,text,text,text,numeric,integer,text,integer,integer,text,text[],boolean,boolean,boolean,integer);
...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      EXISTS (
        SELECT 1 FROM public.screening_questions sq WHERE sq.job_id = j.id
      ) AS has_screening
    FROM public.jobs j
...
GRANT EXECUTE ON FUNCTION public.search_jobs(text,text,text,text,text,numeric,integer,text,integer,integer,text,text[],boolean,boolean,boolean,integer) TO anon, authenticated;