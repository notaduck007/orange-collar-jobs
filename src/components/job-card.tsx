...
import { jobSlugApplyLink } from "@/lib/routes/job-slug-link";
...
export function JobCard({ job }: { job: JobSummary }) {
  ...
  const showApplyControl = !applied;
  const canQuickApply = !!user && quickApply.ready && !job.has_screening;
  ...
  const categoryLine = [job.category, shiftLabel[job.shift]].filter(Boolean).join(" • ");

  return (
    <>
      <article className="group relative rounded-lg border border-border bg-card p-5 shadow-[var(--shadow-card)] transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-[var(--shadow-card-hover)]">
        {slug ? (
          <Link
            to="/jobs/$slug"
            params={{ slug }}
        ...
        {job.featured && (
          <div className="pointer-events-none absolute -top-px right-4 z-[1] flex items-center gap-1 rounded-b-md bg-[color:var(--hazard)] px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-[color:var(--ink)]">
            <Zap className="h-3 w-3" fill="currentColor" aria-hidden /> Featured
          </div>
        )}
        <div className="relative z-[1] pointer-events-none">
          {categoryLine && <p className="label-caps">{categoryLine}</p>}
            ...
            <div className="mt-0.5 flex items-center gap-1.5">
              {job.companies.slug ? (
                <Link
                  ...
                  className="pointer-events-auto relative z-10 text-sm font-medium text-foreground hover:text-primary hover:underline"
                >
                  {job.companies.name}
                </Link>
              ) : (
