...
function buildMockApp(): MockApp {
  const mock: MockApp = {
    ...
    useGlobalPipes: jest.fn((...args: unknown[]) => {
      mock._pipes.push(...args);
    }),
    useGlobalFilters: jest.fn((...args: unknown[]) => {
      mock._filters.push(...args);
    }),
    setGlobalPrefix: jest.fn((prefix: string) => {
      mock._prefix = prefix;
    }),
    enableVersioning: jest.fn((opts: Record<string, unknown>) => {
      mock._versioningOptions = opts;
    }),
  };
  return mock;
}
