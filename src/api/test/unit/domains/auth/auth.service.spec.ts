...
describe('AuthService', () => {
  ...
  describe('login', () => {
    ...
    it('throws UnauthorizedError when email not verified', async () => {
      (prisma.user.findUnique as jest.Mock).mockResolvedValue({ ...baseUser, emailVerifiedAt: null });
      await expect(
  ...
  describe('resetPassword', () => {
    ...
    it('throws BadRequestError for expired reset token', async () => {
      ...
      await expect(service.resetPassword('reset-tok', 'NewSecure1!')).rejects.toThrow(BadRequestError);
    });
  });
